// ==============================
// Analog Archive — optimized for GitHub Pages
// ==============================
...
(function runLoader(){
  if (prefersReducedMotion){ window.addEventListener('load', finishLoading); return; }
  let progress = 0;
...
function pulseTransition(fn){
  if (prefersReducedMotion || !pageTransition){ fn && fn(); return; }
...
function cardTemplate(p){
  ...
  <article class="card" data-id="${escapeHtml(p.id||'')}" data-category="${escapeHtml(p.category||'misc')}" data-featured="${p.featured ? 'true' : 'false'}" data-type="${escapeHtml(p.type||'image')}"
    ...
    <div class="media">
      <img src="${escapeHtml(p.thumb||'')}" alt="${escapeHtml((p.title||'Project') + ' thumbnail')}" loading="lazy" decoding="async" />
...
function bindSearch(){
  if (!projectSearch || projectSearch.dataset.bound) return;
...
// Update <meta> and <link> SEO tags from config
function applySeoMeta(cfg){
  const title = cfg.siteTitle ? `${cfg.siteTitle} — Analog Archive` : document.title;
  const desc = cfg.siteDescription || cfg.about?.description || '';
  const ogImg = cfg.ogImage || cfg.about?.profilePhoto || '';
  const canonical = cfg.canonicalUrl || '';

  document.title = title;

  const setContent = (id, val) => { const el = document.getElementById(id); if (el && val) el.setAttribute('content', val); };
  ...
  if (ogImg) setContent('ogImage', ogImg);
  setContent('twTitle', title);
  setContent('twDescription', desc);
  if (ogImg) setContent('twImage', ogImg);
  if (canonical){ setContent('ogUrl', canonical); setHref('canonicalLink', canonical); }
}
...
function applySiteConfig(cfg){
  ...
  // Auto-update footer year + name
  const footer = document.querySelector('.site-footer p');
  if (footer && cfg.siteTitle) footer.textContent = `© ${new Date().getFullYear()} ${cfg.siteTitle} — Analog Archive`;
  ...
  if (aboutSection){
    ...
    if (about.enabled === false){
      ...
      if (aboutActions){
        ...
        const socialHtml = links.filter(l => l && l.label && l.url).map(l => {
  ...
  if (contactSection){
    ...
    if (c.enabled === false){
      ...
      if (emailBtn){ emailBtn.textContent = (c.emailLabel || 'Email').toUpperCase(); emailBtn.href = `mailto:${email}`; }
      ...
      if (copyBtn){
        ...
        copyBtn.addEventListener('click', async () => {
          ...
          catch(e){ window.prompt('Copy this email:', email); }
        }, { once: true });
      }

      const resumeBtn = document.getElementById('resumeBtn');
      if (resumeBtn){
        if (c.resumeUrl){ resumeBtn.href = c.resumeUrl; resumeBtn.textContent = (c.resumeLabel || 'Resume').toUpperCase(); resumeBtn.style.display = ''; }
      ...
      const bts = cfg.bts || {};
      if (btsLink){
        if (bts.enabled === false || !bts.url) btsLink.style.display = 'none';
        else { btsLink.style.display = ''; btsLink.href = bts.url; btsLink.textContent = (bts.label || 'Behind the Scenes').toUpperCase(); }
      }
      ...
      if (form && enableForm){
        ...
        form.addEventListener('submit', async (ev) => {
          ev.preventDefault();
          if (!endpoint){ status && (status.textContent = 'Add Formspree endpoint in site-config.json'); showToast('Add Formspree endpoint', 1800); return; }
          status && (status.textContent = 'Sending…');
          try{
            const res = await fetch(endpoint, { method:'POST', body: new FormData(form), headers:{'Accept':'application/json'} });
            const msg = res.ok ? (c.successMessage || 'Sent.') : (c.errorMessage || 'Error.');
            if (res.ok) form.reset();
            status && (status.textContent = msg);
            showToast(msg, res.ok ? 1400 : 1800);
          }catch(e){
...
async function loadProjects(){
  ...
  try {
    ...
    const yearNum = (p) => { const m = String(p.year||'').match(/\d{4}/); const n = parseInt(m ? m[0] : '0', 10); return Number.isFinite(n) ? n : 0; };
    const manualOrder = (p, idx) => { const n = Number(p.sortOrder); return Number.isFinite(n) ? n : idx + 9999; };
    projects = projects.slice().sort((a,b) => {
      switch(activeSort){
        ...
        default:         return manualOrder(a,0) - manualOrder(b,0);
      }
    });
    ...
    projectGrid.innerHTML = `
      <article class="card">
        <div class="media" style="padding:20px;cursor:default"><p style="font-size:12px;line-height:1.5;color:var(--muted)">
          Projects could not load. If testing locally, use a local server (not file://). GitHub Pages works normally.
        </p></div>
...
async function btsGate(cfg){
  ...
  document.getElementById('backHomeBtn')?.addEventListener('click', () => pulseTransition(() => window.location.href = 'index.html'));
  document.getElementById('lockBtsBtn')?.addEventListener('click', () => { localStorage.removeItem(key); showToast('Locked'); setTimeout(() => window.location.href = 'index.html', 300); });
  if (localStorage.getItem(key) === 'true'){ document.body.classList.remove('bts-locked'); return; }
  const pw = window.prompt('Enter password.' + (bts.hint ? `\nHint: ${bts.hint}` : ''));
  if (!pw){ window.location.href = 'index.html'; return; }
  try{
    const hash = await sha256Hex(String(bts.salt || '') + pw);
    if (hash === String(bts.passwordHash || '')){ localStorage.setItem(key, 'true'); document.body.classList.remove('bts-locked'); showToast('Access granted'); }
    else { showToast('Wrong password', 1800); window.location.href = 'index.html'; }
  }catch(e){ showToast('Unable to verify', 1800); window.location.href = 'index.html'; }
}
