...
## Replacing sample media (projects + BTS)
All placeholders live in `assets/`. Replace them with your own files.

### Easiest method (recommended)
Replace the files but **keep the same filenames**, for example:
- `assets/sample-photo.svg`
- `assets/sample-design.svg`
- `assets/sample-video-thumb.svg`
- `assets/bts-process-01.svg` (and other `bts-*.svg`)

This way you usually do **not** need to edit any code or JSON.

### If you want different filenames
Update:
- `data/projects.json` → `thumb`, `viewerSrc`, and `gallery`
- `bts.html` → any `assets/bts-*.svg` references
