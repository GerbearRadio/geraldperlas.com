...
## Social links + resume + contact section (new)
Your homepage now includes:
- **social links** under the About section (Instagram, LinkedIn, Email, etc.)
...
```json
{
  ...
  "contact": {
    "enabled": true,
    "heading": "Contact",
    "description": "Open to freelance, collaborations, and full-time opportunities.",
...
1. Put your PDF in `assets/` (example: `assets/my-resume.pdf`)
2. Update `contact.resumeUrl` in `data/site-config.json`
...
The site shows a small toast message at the bottom for:
- Copy Email success
- Contact form success / error

You don’t need to edit anything for this feature.


---

## Troubleshooting: “Projects could not load”
This message appears when the site can’t fetch:

- `data/projects.json`

### Quick test
Open this in your browser (use your real domain):
- `/data/projects.json`

Example:
- `https://geraldperlas.com/data/projects.json`

If it 404s, GitHub Pages is publishing from the wrong branch/folder or the file isn’t at the published root.

### Fix included
This build uses a `BASE_PATH` resolver so JSON and assets work on:
- Custom domains (root `/`)
- GitHub project URLs (`/repo/`)


---

## Behind the Scenes page (password gated)
There is a BTS page at `bts.html`.

### Configure it
Edit `data/site-config.json` → `bts`:

- `enabled`: true/false
- `passwordHash`: sha256(salt + password)
- `salt`: any short string
- `hint`: optional hint text in the prompt
