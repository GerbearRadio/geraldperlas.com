...
## Behind the Scenes | Extras (password gate)

This site is GitHub Pages–friendly, so the "password protection" is a **lightweight, client-side gate** (meant to deter casual visitors, not secure sensitive files).

### Where it lives
- Page: `behind-the-scenes.html`
- Config: `data/site-config.json` → `extras`
- Content list: `data/extras.json`
- Media files: `assets/`

### Change the password
In `data/site-config.json`, update:
- `extras.salt` (any random string)
- `extras.passwordHash` = SHA-256 hex of `${salt}:${password}`

You can generate a hash with a one-liner:
...
### Add extras items
Add entries to `data/extras.json` like:
```json
[
  {
    "project": "Urban Silence",
    "type": "video",
    "title": "Storyboard animatic",
    "description": "Early pacing + beat mapping.",
    "src": "assets/extras/urban-silence-animatic.mp4",
    "thumb": "assets/extras/urban-silence-thumb.jpg",
    "tags": ["storyboard", "animatic"],
    "year": "2026"
  }
]
```
...
### Lock timeout (auto-lock)
You can auto-lock the Extras page after a period of time.

In `data/site-config.json` → `extras`:

- `lockTimeoutMinutes`: locks after X minutes since unlocking (even if active)
- `idleTimeoutMinutes`: locks after X minutes of no activity (mouse/scroll/key)

Set either value to `0` to disable that timeout.

### Categories (tabs)
Extras items support a `category` field, which powers the tabs:
...
### Reusable Extras item template blocks (copy/paste)

**1) Video (Unlisted YouTube embed)**
```json
{
  "title": "Project Name — BTS Reel",
  "category": "video",
  "type": "video",
  "project": "Project Name",
  "year": "2026",
  "description": "Lighting notes + alternate takes.",
  "src": "https://www.youtube.com/embed/VIDEO_ID",
  "thumb": "assets/extras/thumbs/project-bts.jpg",
  "tags": ["bts", "edit", "lighting"]
}
```

**2) Concept Art (local image in `/assets`)**
```json
{
  "title": "Keyframe Concepts",
  "category": "concept-art",
  "type": "image",
  "project": "Project Name",
  "description": "Early mood + palette explorations.",
  "src": "assets/extras/concept/keyframes-01.jpg",
  "tags": ["concept", "palette"]
}
```

**3) Storyboards (Google Drive PDF preview)**
```json
{
  ...
  "type": "pdf",
  "project": "Project Name",
  "description": "Shot plan + timing notes.",
  "src": "https://drive.google.com/file/d/FILE_ID/preview",
  "tags": ["storyboard", "shots"]
}
```

**4) Scripts (PDF preview or external link)**
```json
{
  ...
  "type": "pdf",
  "project": "Project Name",
  "description": "Dialogue + revision notes.",
  "src": "https://drive.google.com/file/d/FILE_ID/preview",
  "tags": ["script", "draft"]
}
```
...
## Method #4 (free + safer): host Extras assets off-site

This keeps behind-the-scenes media **out of your public GitHub repo**.

### A) Videos → YouTube (Unlisted)
1. Upload to YouTube
2. Set visibility to **Unlisted**
3. Use the embed URL format on your Extras item:
   - `https://www.youtube.com/embed/VIDEO_ID`

### B) PDFs / Scripts / Storyboards → Google Drive
1. Upload to Drive
2. Right click → **Get link**
3. Set to: “Anyone with the link” → **Viewer**
4. Use the preview URL format in `extras.json`:
   - `https://drive.google.com/file/d/FILE_ID/preview`

### C) Concept art images → Google Drive (optional)
If you want images *not* stored in GitHub, use:
- `https://drive.google.com/uc?export=view&id=FILE_ID`

> Reminder: the site’s password gate is a “soft lock.” Off-site hosting gives you better control over the files themselves (still free).
